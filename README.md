...
- **Границы стихотворения**: весь текст между `<&>` данного стихотворения и либо следующим `<&>` (начало нового стихотворения), либо новым `<n>` (заглавие нового стихотворения), либо новым `<a>` (смена автора), либо концом документа считается **текстом одного стихотворения**. В случае, если перед строкой с `<&>` есть строка с `<n>`, то 
...
- **Контейнер**. Начинается обязательно с элемента `incipit` (`<&>`) или необязательного `title` (`<n>`).  
...
document
  └── author (один или много)
        ...
        └── poem+ (если нет циклов)
              ├── title?
              ├── prose*
              ├── ( stanza* | line* ) (или строфы, или строки без строф)
              ├── prose*
              ├── date?
              └── place?
```
...
Репозиторий содежит код на языке Python для валидации документа, размеченного в соответствии с синтаксисом VML. Запуск валидатора:
...
### **Веймелка** (произносится: *вэй-мэл-ка*)
...
- Удачная параллель с функциями языка: вратарь «защищает ворота» от неструктурированного текста, а VML «отражает» смысловые удары, выделяя границы стихотворных элементов.
...
```
PoemBlock        = TitleBlock? , IncipitLine , { StructuralElement } , [ DatePlaceBlock ] ;
